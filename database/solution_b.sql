...
--3
SELECT orders.order_id, COUNT(order_items.product_id) AS total_products 
FROM orders 
...
--4
SELECT users.user_id, users.name, orders.order_id, GROUP_CONCAT(products.product_name) AS product_names
FROM users 
JOIN orders ON users.user_id = orders.user_id
JOIN order_items ON orders.order_id = order_items.order_id
JOIN products ON order_items.product_id = products.product_id
GROUP BY orders.order_id, users.user_id, users.name;
...
--7
SELECT users.user_id, users.name, orders.order_id, SUM(order_items.price * order_items.quantity) AS total_amount
FROM users
JOIN orders ON users.user_id = orders.user_id
JOIN order_items ON orders.order_id = order_items.order_id
GROUP BY orders.order_id, users.user_id, users.name;
...
--8
SELECT u.user_id, u.name, o.order_id, MAX(total_amount) AS max_order_amount
FROM (
    SELECT orders.order_id, users.user_id, users.name, SUM(order_items.price * order_items.quantity) AS total_amount
    FROM users
    JOIN orders ON users.user_id = orders.user_id
    JOIN order_items ON orders.order_id = order_items.order_id
    GROUP BY orders.order_id, users.user_id, users.name
...
--9
SELECT u.user_id, u.name, o.order_id, MIN(total_amount) AS min_order_amount, total_products
FROM (
    SELECT orders.order_id, users.user_id, users.name, SUM(order_items.price * order_items.quantity) AS total_amount, COUNT(order_items.product_id) AS total_products
    FROM users
    JOIN orders ON users.user_id = orders.user_id
    JOIN order_items ON orders.order_id = order_items.order_id
    GROUP BY orders.order_id, users.user_id, users.name
...
--10
SELECT u.user_id, u.name, o.order_id, total_amount, MAX(total_products) AS max_products
FROM (
    SELECT orders.order_id, users.user_id, users.name, SUM(order_items.price * order_items.quantity) AS total_amount, COUNT(order_items.product_id) AS total_products
    FROM users
    JOIN orders ON users.user_id = orders.user_id
